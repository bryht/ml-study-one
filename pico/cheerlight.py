...
WIFI_SSID = "Thomson4DC1EC24"
WIFI_PASS = "5607D5B1FA"
init=picowireless.init()
print("Connecting to {}".format(WIFI_SSID))
# picowireless.start_scan_networks()
# while True:
#     result=picowireless.get_scan_networks()
#     if result != 0:
#         break
#     print(result)
#     utime.sleep(3)
...
while True:
    if picowireless.get_connection_status() == 3:
        break
    print(picowireless.get_connection_status())
    utime.sleep(3)
print("Connected!")
...
while True:
    http_request(client_sock, http_address, HTTP_PORT,
                 HTTP_REQUEST_HOST, HTTP_REQUEST_PATH, handler)
    utime.sleep(60.0)
